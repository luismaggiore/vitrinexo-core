...
<h2>Política de Privacidad de Vitrinexo</h2>
<p><strong>Última actualización:</strong> julio de 2026</p>

<h3>1. ¿Quiénes somos?</h3>
<p>Vitrinexo es una plataforma de directorio B2B para empresas de servicios profesionales en Latinoamérica, desarrollada y operada por Maggiore Marketing SpA. Puedes contactarnos en <a href="mailto:hola@vitrinexo.com">hola@vitrinexo.com</a>.</p>
...
<p>El contenido, diseño y marca de Vitrinexo son propiedad de Maggiore Marketing SpA. Los miembros conservan los derechos sobre el contenido que publican en la plataforma.</p>
...
// Create or update Privacidad page
$privacidad = get_page_by_path('privacidad');
if ($privacidad) {
    wp_update_post(['ID' => $privacidad->ID, 'post_content' => $privacidad_content, 'post_status' => 'publish']);
    echo "Updated privacidad: " . $privacidad->ID . PHP_EOL;
} else {
    $id = wp_insert_post(['post_title' => 'Política de Privacidad', 'post_name' => 'privacidad', 'post_content' => $privacidad_content, 'post_status' => 'publish', 'post_type' => 'page']);
    echo "Created privacidad: $id" . PHP_EOL;
}

// Create or update Términos page
$terminos = get_page_by_path('terminos');
if ($terminos) {
    wp_update_post(['ID' => $terminos->ID, 'post_content' => $terminos_content, 'post_status' => 'publish']);
    echo "Updated terminos: " . $terminos->ID . PHP_EOL;
} else {
    $id = wp_insert_post(['post_title' => 'Términos y Condiciones', 'post_name' => 'terminos', 'post_content' => $terminos_content, 'post_status' => 'publish', 'post_type' => 'page']);
    echo "Created terminos: $id" . PHP_EOL;
}
